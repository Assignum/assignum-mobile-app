...
class _InviteMembersPageState extends State<InviteMembersPage> {
  ...
  void _addEmail() {
    ...
    if (text.isNotEmpty && text.contains('@')) {
      setState(() {
        if (!_emails.contains(text)) _emails.add(text);
        _emailCtrl.clear();
      });
    } else {
  ...
  void _removeEmail(int index) => setState(() => _emails.removeAt(index));

  Future<void> _submit() async {
    if (_emails.isEmpty) {
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => ActivityDetailsPage(activity: widget.activity, isCreationFlow: true)));
      return;
    }
                  ...
                  onPressed: () async {
                    ...
                    if (mounted) {
                      Navigator.pushReplacement(context, MaterialPageRoute(
                        builder: (_) => ActivityDetailsPage(activity: updated ?? widget.activity, isCreationFlow: true),
                      ));
                    }
                  },
                ),
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Invita a tus compañeros', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              const SizedBox(height: 24),
              CardContainer(
                child: Column(
                  ...
                  children: [
                    const Padding(
                      ...
                      child: Text('Correo electronico', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                    ),
                    TextFormField(
                      controller: _emailCtrl,
                      decoration: _customDecoration().copyWith(
                        suffixIcon: IconButton(icon: const Icon(Icons.add, color: Colors.white, weight: 800), onPressed: _addEmail),
                      ),
                    ...
                    if (_emails.isNotEmpty)
                      Wrap(
                        ...
                        children: _emails.asMap().entries.map((entry) {
                          return Chip(label: Text(entry.value), onDeleted: () => _removeEmail(entry.key), deleteIconColor: Colors.redAccent);
                        }).toList(),
                      ),
                    const SizedBox(height: 100),
                    PrimaryButton(text: _saving ? 'Enviando...' : 'Siguiente', onPressed: _saving ? null : _submit),
                  ],
                ),
              ),
            ],
